...
class Prtc : InstructionBase
{
    ...
    public override bool Run(ADDRESSING_MODES addresingMode, ref MEM memory, ref CPU processor, ref ClientTasks clientTasks)
    {
        ...
        case ADDRESSING_MODES.IMMEDIATE:
        {
            ...
            clientTasks.consoleBuffer += System.Text.Encoding.UTF8.GetChars(new byte[] {character})[0];
        }break;
        case ADDRESSING_MODES.REGISTRY:
        {
            byte sourceEncoding = processor.ReadByte(memory);

            clientTasks.consoleBuffer += System.Text.Encoding.UTF8.GetChars(new byte[] {(byte)processor.getRegisterValue(sourceEncoding)})[0];
        }break;
        case ADDRESSING_MODES.REGISTRY_POINTER:
        {
            byte registerWithPointer = processor.ReadByte(memory);

            clientTasks.consoleBuffer += System.Text.Encoding.UTF8.GetChars(new byte[] {memory[(UInt16) processor.getRegisterValue(registerWithPointer)]})[0];
        }break;
